...
let prettyPrintPreferences (teacher: Teacher) : string =
    ...
    let prettyPrintGeneralPreferences preference =
        ...
        | IntersectTimeWithTeacher(teacherName, priority) ->
            ...
            let patronymic = 
                ...
                | Some patr -> patr
                | None -> ""
            sprintf "%s пересекать время пар с преподавателем %s %s %s\n" priority teacherName.Surname teacherName.Name patronymic
        | Gaps priority ->
            let priority = prettyPrintPriority priority
    ...
    let prettyPrintTime (dayOfWeekOption) (timeSlotOption) = 
        ...
        let prettyPrintTimeSlot time =
            ...
            | ((startHour, startMinutes), (finalTime, finalMinutes), priority) ->
                let priority = prettyPrintPriority priority
                sprintf "  %s ставить с %s до %s\n" priority (formatTime (startHour, startMinutes)) (formatTime (finalTime, finalMinutes))
        ...
        let timeSlotsStr =
            ...
            | Some(times) -> times |> List.map prettyPrintTimeSlot |> String.concat ""
            | None -> ""
    ...
    let prettyPrintSubjectOrGroupPreferences (groupOption, subjectOption, preference) =
        ...
        let targetStr =
            ...
            | group, (subject, classtype) ->
                sprintf "  %s для групп(ы) %s по предмету %s:\n" (prettyPrintClassType classtype) (colorizeText group 64) (colorizeText subject 64)
    ...
    let header = sprintf "Пожелания преподавателя %s %s %s, кафедра %s:\n" 
    ...
    let preferencesStr =
        ...
        |> List.map (fun preference ->
            ...
            | GeneralPreference generalPreference ->
                prettyPrintGeneralPreferences generalPreference
