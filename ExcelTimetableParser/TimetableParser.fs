...
module ExcelParser =
    ...
    let rec fromIEnumerator<'T> (enumerator : IEnumerator) =
        seq { if enumerator.MoveNext() then enumerator.Current :?> 'T; yield! fromIEnumerator enumerator }
         |> Seq.toList
    ...
    let listOfMatches (matches : MatchCollection) =
        matches.GetEnumerator() |> fromIEnumerator<Match> |> Seq.map string |> Seq.toList

    let (|Date|_|) (str: string) = if str.ToLower().Trim() = "день" then Some () else None
    let (|Time|_|) (str: string) = if str.ToLower().Trim() = "время" then Some () else None
    ...
    let rec parseHeader startRow (worksheet: IXLWorksheet) =
        ...
        let usedCells = potentialHeaderRow.CellsUsed() |> cellsToSeq |> Seq.filter cellIsMeaningful
        if Seq.length usedCells < 2 then parseHeader (startRow + 1) worksheet
        else (startRow, potentialHeaderRow)


    // Находим лекторов Фамилия И.О. или Фамилия ИО
    let tryFetchLecturer (str : string) =
        let pattern = @"([А-Я][а-я]* [А-Я]\.[А-Я]\.)|([А-Я][а-я]* [А-Я]\.[А-Я])|([А-Я][а-я]* [А-Я][А-Я]\ )";
    ...
    let weekdayOfString (s : string) (errors: ResizeArray<string>) : DayOfWeek =
        ...
        let processed = alphas.Matches(s.ToLower()) |> listOfMatches |> List.head;
        ...
        | "пятница" -> Friday
        | "суббота" -> Saturday
        | _ ->  errors.Add($"Не удалось распознать {s} как день недели. Было вставлено воскресенье")
                Sunday
    ...
    let convertStringToTime (weekDay : string) (str : string) (cell : IXLCell) (errors: ResizeArray<string>) =
        ...
        | [s; e] -> Some {Weekday = weekDay; StartTime = (convTime s); EndTime = (convTime e)}
        | [s] -> Some {Weekday = weekDay; StartTime = (convTime s); EndTime = (convTime s)}
        | _ -> errors.Add($"Не выходит распознать время из ячейки {cell.Address.ToString()} {str}"); None


    let fetchActivity (str : string) =
        let mutable result = NoClassType
        if Regex.Matches(str, @"(\(пр\. з\.\))|(прак)", RegexOptions.IgnoreCase).Count > 0 then result <- Practical
        if Regex.Matches(str, @"\(лекц\.\)|(лекц)", RegexOptions.IgnoreCase).Count > 0 then result <- Lecture
        result
    ...
    let processLecture (cell : IXLCell) (str : string) (weekDay : string) (timeString : string) (errors: ResizeArray<string>) : TimetableSlot option =
        let teachers = tryFetchLecturer str
        if teachers.Count = 0 then errors.Add($"Преподаватель не найден для {str}")
        let teachers = teachers |> listOfMatches

        let auditoriums = tryFetchAuditorium str
        if auditoriums.Count = 0 then errors.Add($"Аудитория не найдена для {str}")
        let auditoriums = auditoriums |> listOfMatches |> Seq.map Int32.Parse |> Seq.distinct

        let auditorium = auditoriums |> Seq.tryHead |> someOr 0
        if auditoriums |> Seq.length > 1 then errors.Add($"Было найдено несколько аудиторий %A{auditoriums}. Была выбрана первая: {auditorium}")
        ...
        let processCell (cell : IXLCell) =
            ...
            | Group g, _ ->
                ...
                | Some lectureEntry ->
                    ...
                    | Some time ->
                        tt.TryAdd((time, g.ToString()), lectureEntry) |> ignore
            ...
            | _ -> errors.Add($"Ячейка {headerRange.FirstAddress.ToString()} со значением {headerValue} не является группой или датой/временем")



        let cellSeq = row.Cells() |> cellsToSeq |> Seq.filter cellIsMeaningful |> flattenSubCells |> Seq.toList
    ...
    let processFile (filePath: string) : Result<ParsingResult, string> =
        try
            ...
            Ok {Timetable = timetable; Errors = List.ofSeq parsingErrors}
        with
